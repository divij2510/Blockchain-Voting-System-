...
class VerifyEmailView(APIView):
    def post(self, request):
        ...
        try:
            voter = Voter.objects.get(email_id=email)
        except Voter.DoesNotExist:
...
class AddVoterView(APIView):
    def post(self, request):
        ...
        for email in emails:
            Voter.objects.create(email_id=email)
...
class GetCandidatesView(APIView):
    def get(self, request):
        ...
        if candidates is None:
            return Response({'error':'No candidates to display'}, status=status.HTTP_204_NO_CONTENT)
